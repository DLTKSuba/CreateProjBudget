...
export function useRightSidebarPanel(options: UseRightSidebarPanelOptions = {}) {
  ...
  const handleItemActivate = useCallback(
    (detail: RightSidebarItemActivateDetail) => {
      ...
      // Clicking the active rail item again collapses the panel.
      if (open && activeItemKey === itemKey) {
        closePanel()
        return
      }
    ...
    [open, activeItemKey, closePanel, shellPanelScopeRef, rightPanelRef, shellPanelVariant]
  )
